...
int Check_studentid();//学生登录菜单
int Check_teacherid();//图书管理员登陆菜单
void BORROW(char[]); //借书模块
int borrow_buffer(char[]); //借书_购物车
void borrow_buffer_change_lend(char[]);//借书_购物车_更改图书lend（借阅状态）
int borrow_borrow(char[]);//借书_正式借出
void borrow_borrow_load(char[],char[]);//借书_正式借出_更改文件信息
void borrow_borrow_delet(char[],char[]);//借书_正式借出_删除购物车记录
void borrow_borrow_change_borrow_book_num(char[]);//借书_ 正式借出_更改学生外借图书数量
void borrow_borrow_change_lendtime(char[]);//借书_正式借出_ 更改图书被借次数
int borrowtime(char[]);//对学生判断是否超时超时0，不超时1
void renew(char[]);//续借
int Return(char[]);//还书
void Return_change_lend(char[]);//还书_更改图书lend（借阅状态）
void Return_change_return_date(char[],char[]);//还书_更改_还书日期
void Return_change_borrow_book_num(char[]);//还书_更改_学生外借图书数量
void teacher_enrolment(void); //图书管理员注册
void student_enrolment(void);//学生注册
int idnumbervalidation(char[]);//学号/工号格式判断
int phonenumbervalidation(char[]);//电话号码格式判断
int mailboxvalidation(char[]);//邮箱格式判断
int booknumbervalidation(char[]);//图书编号格式判断
void teacherinfo_management(char[]);//图书管理员个人信息管理
void studentinfo_management(char[]);//学生个人信息管理
void deleteborrowinfo(char[]);//删除购物车记录
int search_by_name();//通过书名查询
int search_by_writername();//通过作者查询
int search_by_press();//通过出版社查询
int book_add();//添加图书
int book_delete();//删除图书
int update();//更改图书
int Borrow_Time_Overdue_Book_Situation();//超时未还图书
int Borrow_Time_Overdue_Students_Situation();//超时未还行为学生
int Borrow_Time_Overdue_Frozen();//冻结学生账号
int Borrow_Time_Overdue_Frozen1();//解冻学生账号
void Frozen_Account(char[]);//冻结_修改Frozen
void Frozen_Account1(char[]);//解冻_修改Frozen
int BOOK_Borrow_situation();//图书借阅情况查询
int Search_By_Student_Name_Or_Title();//通过学生姓名或书名查询
int Sort_borrow_number();//通过借阅次数排序
int Sort_borrow_time();//通过借阅时间长短排序
int Sort_By_Book_Price();//通过价格排序
int bookname_borrowtime(char[]);//计算出每一本书的借阅总时长
int student_borrow_history(char[]);//学生借阅历史查询
void renew_a_book(char[]);//续借
void renew_a_book_change_borrowtime_and_renew_num(char[],char[]);//续借_更改图书借书时间以及续借次数
int borrowtime_booknumber(char[]);//对单个书本判断是否超时超时0，不超时1
...
//						system("cls");
                        ...
                        switch(b)
                        {
                            ...
                            case 3://查询图书 ok
                            {
                                ...
                                {
                                    system("cls");
                                    Display_student_book_searchmenu();//学生图书查询菜单
                                    scanf("%d",&b1);
                                    switch(b1)
                                    {
                                        case 1://根据书名查询 ok
